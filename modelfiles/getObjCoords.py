...
# Select relevant emotions
selected_emotions = ['admiration', 'excitement', 'gratitude', 'joy', 'love']
...
def get_coords(emotion_pred_vector):
    # Filter emotions_map_labels to include only selected_emotions
    filtered_emotions_map_labels = {key: emotions_map_labels[key] for key in selected_emotions}
    
    # Transform filtered_emotions_map_labels values into a (5, 2) array
    emotions_matrix = np.array(list(filtered_emotions_map_labels.values()))
...
# Ensure the vector has the correct shape (1, 5)
emotion_pred_vector = emotion_pred_vector.reshape(1, -1)

# Call the function with the hardcoded object
coords = get_coords(emotion_pred_vector)
print(f"Resulting coordinates: {coords}")
